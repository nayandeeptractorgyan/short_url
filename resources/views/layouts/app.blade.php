...
@auth
<nav>
    <a href="{{ route('dashboard') }}" class="brand">🔗 URL Shortener</a>
    <a href="{{ route('dashboard') }}">Dashboard</a>
    <a href="{{ route('short-urls.index') }}">URLs</a>
    @if(auth()->user()->role->canInvite())
        <a href="{{ route('invitations.index') }}">Invitations</a>
    @endif
    <div class="user-info">
        <strong>{{ auth()->user()->name }}</strong>
        <span class="badge badge-{{ auth()->user()->role->value }}">{{ auth()->user()->role->label() }}</span>
